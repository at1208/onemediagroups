    ...
    if (project) {
      setTeamMembers(
        project.team_members
          .filter((val) => val._id !== project.team_leader._id)
          .slice(0, 3)
      );
  ...
  return (
    <Card mb={6}>
      ...
      <Card className={classes.cardRoot}>
        ...
        {taskCount && (
          <Grid container spacing={3} className={classes.chipContainer}>
            <Grid item xs={4} sm={4} md={4} lg={4}>
              <Chip
                label={<>Open task: {taskCount.open}</>}
                className={classes.openChip}
              />
            </Grid>
            <Grid item xs={4} sm={4} md={4} lg={4}>
              <Chip
                label={<>Done task: {taskCount.done}</>}
