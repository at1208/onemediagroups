...
const Chats = ({
  ...
  const handleChange = (e) => {
    ...
    socket.emit(
      "typing",
      { first_name, last_name, email, senderId: id },
      { room: getChannelId(location) }
    );
  };
    ...
    return (
      <div key={i}>
        <ListItem
          ...
          button
          onClick={() =>
            history.push(`/chats/${item.channel_name}?id=${item._id}`)
          }
        >
          <Grid container>
  ...
  const chatsList = chats.map((item, i) => {
    ...
    return (
      <div key={i} className={classes.singleMessage}>
        <Grid container justify="flex-start" spacing={2}>
          <Grid item xs={3} md={1} sm={2}>
            <Grid container justify="flex-start">
              <Avatar
                alt={item.senderName}
                src={item.senderPicture}
                width={40}
                height={40}
              />
            </Grid>
          ...
          <Grid item xs={8} md={10} sm={9}>
            <Grid container justify="flex-start">
              <Grid item xs={12} md={12} sm={12}>
                <Typography variant="body1" className={classes.senderName}>
                  {capitalizeFirstCharacter(item.senderName || "undefined")}
                </Typography>
              </Grid>
              <Grid item xs={12} md={12} sm={12}>
          ...
          <div className={classes.message}>{renderHTML(item.message)}</div>
        </Grid>
      </div>
    );
  });
        ...
        <Grid container justify="center" spacing={0}>
          <Grid item sm={12} md={8} lg={9} xs={12}>
            <Card className={classes.chatDisplay}>
              {chatsList}
              <div ref={(el) => (messageContainer = el)}></div>
            </Card>
            <Typography variant="body2" align="center">
              {typing.status && typing.msg}
            </Typography>
            <br /> <br />
            <form onSubmit={handleSubmit} className={classes.messageRoot}>
              {!matches && (
                <Typography
                  variant="body2"
                  align="center"
                  className={classes.channelNameMb}
                >
                  #{channel}
                </Typography>
              )}
          ...
          {matches && (
            <Grid item xs={12} md={3} sm={12} lg={3}>
              ...
              <Card className={classes.cardRoot}>
                ...
                <br />
                <ChannelMembers
                  pageReload={(status) => setReload(status)}
