...
const Chat = ({
  onlineUsers,
  match: { params: id, url },
  location: { state: selectedReceiver },
}) => {
  ...
  useEffect(() => {
    socket.on("receivePrivateMsg", (receiveMsg) => {
      if (selectedReceiver && isAuth()) {
        if (
          selectedReceiver.selectedReceiver._id + isAuth()._id ===
          receiveMsg.msg.msg.senderId + receiveMsg.msg.msg.receiverId
        ) {
  ...
  const onEnter = async (e) => {
    ...
    let loadMoreMessages = await getPrivateChats(id.id, token, {
      skip: skip,
      limit: limit,
    });
    ...
    return chatList.map((chatMsg, i) => {
      return (
        <div className={classes.msgContainer}>
          <div
            className={
              chatMsg.senderId === (isAuth() && isAuth()._id)
                ? classes.myMsgAlign
                : classes.thereMsgAlign
            }
          >
            <span
              className={
            ...
            <small className={classes.time}>
              {moment(chatMsg.timestamp).format("hh:mm a - M/D/Y")}
            </small>
          </div>
        ...
        <Grid item sm={10} md={9} xs={12} lg={9}>
                  ...
                  <Box pt={0} pl={1}>
                    ...
                    {typing.status ? (
                      <Typography variant="" align="center">
                        <small>{typing.status && typing.msg}</small>
                      </Typography>
          ...
          <Card className={classes.displayMessage}>
            {chatList.length >= 10 && (
              <Grid container justify="center">
                <Grid>
                  <button onClick={onEnter}>Load previous messages</button>
                </Grid>
              </Grid>
            )}
