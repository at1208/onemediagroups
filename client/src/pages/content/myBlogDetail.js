...
import { mySingleBlog } from "../../actions/blog";
...
const BlogDetail = ({
  ...
  useEffect(() => {
    (async () => {
      let permissionCheck = await checkModulePermission(
        "blog",
        "update",
        token
      );
      ...
      <>
        <Grid container justify="space-between">
          <Grid item sm={5}>
            <Box p={2}>
              <ApprovalStatus
                status={blog.approval}
                setValue={(value) =>
                  setReviewSetting({ ...reviewSetting, approval: value })
                }
              />
              <br />
              <LiveStatus
                status={blog.status}
                setValue={(value) =>
                  setReviewSetting({ ...reviewSetting, status: value })
                }
              />
              <br />
              <Grid container justify="center">
          ...
          <Grid item sm={5}>
            <Grid container justify="space-between">
              ...
              <Grid item sm={9}>
                <Chip
                  size="small"
                  label={blog.approval.toLowerCase()}
                  className={
                    blog.approval === "WAITING"
                      ? classes.waiting
                      : blog.approval === "APPROVED"
                      ? classes.approved
                      : blog.approval === "NOT APPROVED"
                      ? classes.notApproved
                      : ""
                  }
                />
            ...
            <Grid container justify="space-between">
              ...
              <Grid item sm={9}>
                <Chip
                  size="small"
                  label={blog.status ? "active" : "inactive"}
                  className={
                    blog.status ? classes.approved : classes.notApproved
                  }
                />
              </Grid>
            </Grid>
          ...
          <Card>
            ...
            <Grid container justify="center" spacing={3}>
              <Grid item sm={8} md={8} xs={12} className={classes.blogContent}>
                ...
                <Link
                  to={{
                    pathname: `/content/myblog/edit/${blog._id}`,
                    state: blog,
                  }}
                >
                  <Button
